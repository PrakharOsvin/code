...
foreach ($mapNotifications as $key => $value) {
  // echo $value->status;
        ...
        echo '<a href = '.base_url().'Dashboard/trip_info/'.$value->id.'>Ride Details</a></br>';
        echo "Trip Id : <br>Passenger name:<a href='".base_url().'Dashboard/profile/'.$value->id."'> ".$value->passenger."</a>";
        if($value->status == 4){
        ...
        echo "<br>Driver name:<a href='".base_url().'Dashboard/'."profile/$value->driver_id'>".$value->driver."</a><br>Pickup: ".$value->pickup_location."<br>Vehicle Type: ".$value->client."<br>Destination: ".$value->dropoff_location."<br>Date: ".$value->modified_on;
...
function markers2(){
  ...
  $.ajax({
      type: "GET",
      url: "<?php echo base_url() ?>Dashboard/markerUser/<?php echo $user_id; ?>",
      data: '',
          ...
          // for (var i = 0; i < data.length; i++) {
            // console.log(data[i]["longitude"]);
              var marker2 = new google.maps.Marker({
                map: map2,
                draggable: false,
                icon: "http://chart.apis.google.com/chart?chst=d_map_pin_letter&chld="+data["vehicle_permit_type"]+"|9999FF|000000",
                title: "#"+data["id"]+", Name:"+data["name"]+", vehicle type:"+data["vehicle_type"],
                // animation: google.maps.Animation.DROP,
                url: "<?php echo base_url() ?>Dashboard/profile/"+data["id"],
              ...
              if (!map2.getBounds().contains(marker2.getPosition())) 
              { //Note the double &  
              ...
              if (marker2!="") {
              console.log("found");
              marker2.setPosition( new google.maps.LatLng( parseFloat(data["latitude"]), parseFloat(data["longitude"]) ) );
              // map2.panTo( new google.maps.LatLng( parseFloat(data["latitude"]), parseFloat(data["longitude"]) ) );
            }
  ...
  function mapNotifications() {   
      $.ajax(
        {
       type: 'get',
       url: '<?php echo base_url();?>Dashboard/ongoingRide/<?php echo $user_id; ?>',
       data : '',
       dataType:'json',
       success: function(data) 
       {
