...
add_shortcode( 'gallery', 'couponxxl_gallery' );
function couponxxl_gallery( $attr ){
	$post = get_post();
	...
	foreach ( $attachments as $id => $attachment ) {
		...
		$output .= "
			<a class='gallery-item {$orientation}' title='".esc_attr( wptexturize($attachment->post_excerpt) )."' href='".$image_full[0]."'>
